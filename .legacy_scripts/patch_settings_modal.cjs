...
// 1. Add voiceId to SettingsModalProps
code = code.replace(
  /voiceSpeed: number;\n  onVoiceSpeedChange: \(speed: number\) => void;/,
  'voiceSpeed: number;\n  onVoiceSpeedChange: (speed: number) => void;\n  voiceId: string;\n  onVoiceIdChange: (id: string) => void;'
);

// 2. Destructure voiceId and onVoiceIdChange
code = code.replace(
  /onVoiceSpeedChange,\n\}\) => \{/,
  'onVoiceSpeedChange,\n  voiceId,\n  onVoiceIdChange,\n}) => {'
);

// 3. Add Voice Selector UI
const voiceSelectorUI = `
            {/* Voice Selection */}
            <div className="space-y-3 pt-4 border-t border-slate-100">
              <span className="text-xs font-mono uppercase tracking-wider text-slate-500 font-semibold block flex items-center gap-1.5">
