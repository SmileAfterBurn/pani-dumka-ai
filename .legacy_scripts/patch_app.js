...
const chatHeaderTarget = '<div className="flex items-center gap-2">';
...
// Make sure to only replace the first occurrence in the chat view
code = code.replace(
  /(\<div className="flex-1 overflow-hidden flex flex-col bg-slate-50"\>\s*\<header className="h-14 border-b border-slate-100 bg-white flex items-center justify-between px-4"\>\s*\<div className="flex items-center gap-3"\>[\s\S]*?\<\/div>\s*)\<div className="flex items-center gap-2"\>/m,
  `$1${chatHeaderReplacement}`
);
